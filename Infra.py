...
# --- 1. LOCALISATION DU FICHIER (Bureau) ---
...
try:
    ...
    # EXTRACTION SÉCURISÉE (Index corrigés pour header=None)
    # Dans Excel, la ligne 17 devient l'index 16. La colonne D devient l'index 3.
...
def export_to_pdf(resultats_tri, resultats_dscr, filename="Risk_Report_Infra.pdf"):
    # --- CALCULS DES KPIS POUR LA CONCLUSION ---
    ...
    with PdfPages(filename) as pdf:
        ...
        # On crée un texte de conclusion propre
        conclusion_text = (
        ...
        # Ajustement de l'espacement pour que rien ne se chevauche
...
# --- ÉTAPE 1 : DÉFINITION DES SLIDERS (UNE SEULE FOIS) ---
# On les met tout en haut pour qu'ils soient toujours visibles
prix_moyen = st.sidebar.slider("Prix du Baril ($)", 30, 100, 60)
volatilité = st.sidebar.slider("Volatilité du marché (%)", 5, 40, 20) / 100

# --- ÉTAPE 2 : LE BOUTON DE CALCUL ---
if st.sidebar.button("Calculer la Viabilité"):
    
    # --- ÉTAPE 3 : RÉ-GÉNÉRATION DES SCÉNARIOS ---
    # C'est ici que la magie opère : on utilise 'prix_moyen' du slider
    ...
    # --- ÉTAPE 4 : TA BOUCLE FOR (Moteur financier) ---
    for prix in scenarios_prix:
        # On évite les prix aberrants
        prix = max(prix, 1) 
